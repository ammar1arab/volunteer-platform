...
const readRaw = (key: string, fallback: string) => {
  ...
  try {
    return window.localStorage.getItem(key) ?? memoryFallback.get(key) ?? fallback;
  } catch {
...
export function useLocalStorageState<T>(key: string, initialValue: InitialValue<T>) {
  ...
  const defaultRaw = useMemo(() => JSON.stringify(initialRef.current!.value), []);
  ...
  const getSnapshot = useCallback(() => readRaw(key, defaultRaw), [defaultRaw, key]);
