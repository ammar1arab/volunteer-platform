...
import { serviceError, guard, GuardError } from "@/core/application/common";
import { toFeaturedPostDto, toFeaturedPostDtoList } from "@/core/application/mappers";
import {
  ok,
  fail,
...
class FeaturedPostUseCase {
  private static readonly SCOPE = "FeaturedPostUseCase";
  ...
  private async tryDeleteImage(imageUrl: string): Promise<void> {
    try {
      ...
      logger.warn(FeaturedPostUseCase.SCOPE, "tryDeleteImage", `Failed to delete: ${imageUrl}`);
    }
  }
  ...
  async create(dto: CreateFeaturedPostRequest): Promise<CreateFeaturedPostResponse> {
    try {
      ...
      guard(payload.imageUrl, "الصورة مطلوبة");
      guard(payload.title, "العنوان مطلوب");
      guard(payload.description, "الوصف مطلوب");

      if (!payload.categories || payload.categories.length === 0) {
        throw new GuardError(fail("VALIDATION_ERROR", "الفئات مطلوبة"));
      }
      ...
      logger.info(FeaturedPostUseCase.SCOPE, "create", `Post created: ${created.id}`);

      return ok({ post: toFeaturedPostDto(created) });
    } catch (error) {
      return serviceError(FeaturedPostUseCase.SCOPE, "create", error, "حدث خطأ أثناء إنشاء المنشور المميز");
    }
  }

  async update(id: string, dto: Partial<UpdateFeaturedPostRequest>): Promise<UpdateFeaturedPostResponse> {
    try {
      ...
      return serviceError(FeaturedPostUseCase.SCOPE, "update", error, "حدث خطأ أثناء تحديث المنشور المميز");
    }
  }
  ...
  async delete(id: string): Promise<DeleteFeaturedPostResponse> {
    try {
      ...
      logger.info(FeaturedPostUseCase.SCOPE, "delete", `Post deleted: ${id}`);
      return ok({ deleted: true });
    } catch (error) {
      return serviceError(FeaturedPostUseCase.SCOPE, "delete", error, "حدث خطأ أثناء حذف المنشور المميز");
    }
  }
  ...
  async getOne(id: string): Promise<GetFeaturedPostResponse> {
    try {
      ...
      return serviceError(FeaturedPostUseCase.SCOPE, "getOne", error, "حدث خطأ أثناء جلب المنشور المميز");
    }
  }
