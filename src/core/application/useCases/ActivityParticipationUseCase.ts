...
class ActivityParticipationUseCase {
  ...
  async createJoinRequest(activityId: string, volunteerId: string): Promise<CreateJoinRequestResponse> {
    try {
      ...
      if (activity.status !== "PUBLISHED") return fail("INVALID_STATE", "النشاط غير منشور");
      ...
      if (existing) {
        if (existing.status === "PENDING") return fail("CONFLICT", "لديك طلب قيد المراجعة بالفعل");
        if (existing.status === "APPROVED") return fail("CONFLICT", "أنت مشارك بالفعل في هذا النشاط");
      }
