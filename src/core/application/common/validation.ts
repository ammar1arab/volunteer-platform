...
export function guardLength(value: string, min: number, max: number, message: string): void {
  if (value.length < min || value.length > max)
    throw new GuardError(fail("VALIDATION_ERROR", message));
}

export function guardRange(value: number, min: number, max: number, message: string): void {
