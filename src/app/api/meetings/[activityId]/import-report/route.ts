...
export async function POST(req: Request, ctx: { params: Promise<{ activityId: string }> }) {
  try {
    ...
    logger.info("API", "POST /meetings/[activityId]/import-report", activityId);
    return toResponse(await providers.meeting().importReport(activityId));
  } catch (error) {
    return apiError("API", "POST /meetings/[activityId]/import-report", error);
  }
}
